"""COMMENT ON TEXT SEARCH CONFIGURATION uses a real owned local object."""
...
MID='manifest_comment_tsc'
NS='g_comment_tsc_ns'
TARGET='TEXT SEARCH CONFIGURATION g_comment_tsc_ns.simple'


class CommentTextSearchConfigurationTests(unittest.TestCase):
    @classmethod
    def setUpClass(cls):
    ...
    def test_configuration_target_crosses_four_text_values(self):
    ...
    def test_real_schema_configuration_and_reverse_owned_cleanup(self):
        cases,_=self.cases()
        for c in cases:
            self.assertEqual(c.setup_sqls,[
                f'CREATE SCHEMA {NS};',
                f'CREATE TEXT SEARCH CONFIGURATION {NS}.simple (PARSER=default);',
            ])
            self.assertEqual(c.teardown_sqls,[
                f'DROP TEXT SEARCH CONFIGURATION {NS}.simple RESTRICT;',
                f'DROP SCHEMA {NS} RESTRICT;',
            ])
            self.assertFalse(any('CASCADE' in x or 'DROP OWNED' in x
                                 for x in c.setup_sqls+c.teardown_sqls))
        fixture=self.r.fixtures['fixture_comment_tsc']
        self.assertFalse(fixture.provides.tables)
        for phrase in ('非当前模式','本case','setup失败','不调用配置分词'):
    ...
    def test_gate_feature_scenario_and_cross_contract(self):
        ...
        self.assertIn('create_text_search_configuration::create_text_search_configuration_fact_internal',
                      gates['internal_text_search_testing']['fact_refs'])
        graph=self.r.factor_dependency_graph();order=self.r.factor_topological_order()
        self.assertTrue({'create_text_search_configuration','drop_schema'}<=graph['comment'])
        self.assertTrue(all(order.index(d)<order.index('comment')
                            for d in ('create_text_search_configuration','drop_schema')))
        fs={f.id:f for f in self.r.matrices['matrix_comment_coverage'].documented_features}
        feature=fs['comment_feature_object_text_search_configuration']
        self.assertEqual((feature.status,feature.coverage_mode),('covered','representative'))
        self.assertEqual(feature.value_refs,['comment_target_configuration_fresh'])
        audit=FactorCoverageAuditor(self.r).audit('comment')
        self.assertFalse(audit['conclusions']['behavior_coverage_complete'])
        scenario=self.r.scenarios['scenario_comment_tsc']
