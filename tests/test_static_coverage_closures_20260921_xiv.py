"""DDL deletion commands close static syntax coverage."""
...
PACKAGES = {
    "drop_database_link": (
        ...
        "drop_database_link_fact_cleanup_conflict",
        4,
    ),
    "drop_extension": (
        "manifest_drop_extension_existing",
        "drop_extension_fact_dependency_profile",
        6,
    ),
    "drop_foreign_data_wrapper": (
        "manifest_drop_foreign_data_wrapper_existing",
        "drop_foreign_data_wrapper_fact_dependency_profile",
        6,
    ),
    "drop_pluggable_database_including_datafiles": (
        "manifest_drop_pluggable_database_including_datafiles_fresh_syntax",
        "drop_pluggable_database_including_datafiles_fact_destructive_cleanup_contract",
        1,
    ),
}


class StaticCoverageClosureXXITests(unittest.TestCase):
    @classmethod
    def setUpClass(cls):
