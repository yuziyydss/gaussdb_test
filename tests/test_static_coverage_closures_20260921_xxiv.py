"""LLM, DECLARE, aggregate, and foreign table static closures."""
...
PACKAGES = {
    "create_llm": (
        [
            "create_llm_fact_runtime_contract",
            "create_llm_fact_example_secret_length",
            "create_llm_fact_api_path_conflict",
        ],
        3,
    ),
    "declare": (
        [
            "declare_fact_query_gap",
            "declare_fact_block_gap",
            "declare_fact_reverse_gap",
        ],
        8,
    ),
    "drop_aggregate": (
        [
            "drop_aggregate_fact_signature_profile",
            "drop_aggregate_fact_dependent_profile",
        ],
        9,
    ),
    "drop_foreign_table": (
        [
            "drop_foreign_table_fact_dependency_conflict",
            "drop_foreign_table_fact_runtime_fixture",
        ],
        6,
    ),
}
...
class StaticCoverageClosureXXXITests(unittest.TestCase):
    @classmethod
    def setUpClass(cls):
