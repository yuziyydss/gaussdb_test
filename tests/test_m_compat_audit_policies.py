...
class MAuditPolicyTests(unittest.TestCase):
    ...
    def test_reconstruction_and_visible_gaps(self):
        ...
        for key in ('create_audit_policy','alter_audit_policy','drop_audit_policy'):
            p=BUILDERS[key]()
            for name,obj in p.finish().items():assert_evolved_asset(self, (ROOT/'specs/ddl'/p.id/name).read_text(),yaml.safe_dump(obj,allow_unicode=True,sort_keys=False,width=110))
