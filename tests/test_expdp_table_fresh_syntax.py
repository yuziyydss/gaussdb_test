"""EXPDP TABLE uses one static syntax representative."""
...
MID='manifest_expdp_table_fresh_syntax'
TABLE='g_expdp_table'
DIRECTORY='/tmp/gaussdb_static_export'


class ExpdpTableFreshSyntaxTests(unittest.TestCase):
    @classmethod
    def setUpClass(cls):
    ...
    def test_one_static_syntax_candidate_is_generated(self):
        ...
        self.assertEqual(cases[0].sql,
                         f"EXPDP TABLE {TABLE} LOCATION = '{DIRECTORY}';")
    ...
    def test_syntax_feature_is_covered_without_backup_claim(self):
        self.cases()
        features={f.id:f for f in self.r.matrices['matrix_expdp_table_coverage'].documented_features}
        feature=features['expdp_table_feature_syntax']
        self.assertEqual((feature.status,feature.coverage_mode),('covered','any'))
        self.assertEqual(feature.value_refs,[
            'expdp_table_table_name_dedicated',
            'expdp_table_directory_path',
        ])
        audit=FactorCoverageAuditor(self.r).audit('expdp_table')
        self.assertFalse(audit['conclusions']['behavior_coverage_complete'])
        self.assertTrue(audit['conclusions']['static_coverage_complete'])
        self.assertNotIn('expdp_table_feature_physical_backup_restore',
