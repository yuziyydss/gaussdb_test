"""INSERT ALL subquery alias negative uses a source-confirmed error."""
...
MID='manifest_insert_all_subquery_alias_negative'


class InsertAllSubqueryAliasNegativeTests(unittest.TestCase):
    @classmethod
    def setUpClass(cls):
        cls.r=FactorPackageRegistry(ROOT/'specs');cls.r.load_all()
        cls.g=FactorPackageSQLGenerator(cls.r)

    def test_one_subquery_alias_negative_candidate_is_generated(self):
        ...
        self.assertEqual(case.expected_error_category,'subquery_alias_reference')
        self.assertEqual(case.expected_error_regex,
                         'Missing FROM-clause entry for table "source_alias"')
    ...
    def test_rule_is_targeted_and_fixture_is_transaction_scoped(self):
        ...
        rule=next(r for r in self.r.get_factor('insert_all').rules
                  if r.id=='insert_all_rule_no_subquery_alias_reference')
        self.assertEqual(rule.expression,"targets != 'insert_all_targets_alias_reference'")
