"""ALTER RESOURCE POOL static closure with a gated MAX_DOP syntax facet."""
...
class StaticCoverageClosureLVIITests(unittest.TestCase):
    @classmethod
    def setUpClass(cls):
    ...
    def generated(self):
        cases = []
        for manifest_id in self.r.factors["alter_resource_pool"].manifest_refs:
    ...
    def test_ordinary_and_max_dop_candidates_are_preserved(self):
        ...
        self.assertEqual(
            max_dop[0].sql, "ALTER RESOURCE POOL b9_pool WITH (MAX_DOP = 1);"
        )
    ...
    def test_resource_pool_boundaries_are_confirmed_environment(self):
        facts = {fact.id: fact for fact in self.r.get_factor("alter_resource_pool").facts}
        for fact_id in (
            "alter_resource_pool_fact_multitenant_conflict",
            "alter_resource_pool_fact_threshold_conflict",
            "alter_resource_pool_fact_additional_options",
            "alter_resource_pool_fact_dop_centralized_conflict",
        ):
    ...
    def test_alter_resource_pool_closes_static_without_behavior(self):
        self.generated()
        audit = self.auditor.audit("alter_resource_pool")
