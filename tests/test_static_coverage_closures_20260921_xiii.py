"""Model and shutdown commands close static syntax coverage."""
...
PACKAGES = {
    "drop_llm": (
        ...
        "drop_llm_fact_runtime_contract",
        1,
    ),
    "drop_model": (
        "manifest_drop_model_fresh_syntax",
        "drop_model_fact_model_contract",
        1,
    ),
    "predict_by": (
        "manifest_predict_by_fresh_syntax",
        "predict_by_fact_model_contract",
        1,
    ),
    "shutdown": (
        "manifest_shutdown_fresh_default",
        "shutdown_fact_disposable_node",
        3,
    ),
}


class StaticCoverageClosureXXTests(unittest.TestCase):
    @classmethod
    def setUpClass(cls):
