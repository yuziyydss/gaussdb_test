-- generated_from: manifest_create_index_partition_nulls_first_negative
-- static_only: true
-- case_count: 1

-- case_id: manifest_create_index_partition_nulls_first_negative_2f3d2c4cb4ce
-- expected: error
-- expected_error_category: partition_index_nulls_first_not_supported
-- expected_sqlstates: -
-- expected_error_regex: (?i)(nulls first|partition|syntax|not support)
-- params: {"comment_clause": "ci_comment_none", "concurrently": "ci_concurrently_none", "if_not_exists": "ci_if_not_exists_none", "include_profile": "ci_include_none", "index_name_presence": "ci_index_name_present", "key_profile": "ci_key_name_desc_nulls_first", "method": "ci_method_default", "scope_clause": "ci_scope_global", "statement_form": "ci_statement_partition", "storage_profile": "ci_storage_none", "table_profile": "ci_table_partitioned", "tablespace_clause": "ci_tablespace_none", "unique_modifier": "ci_unique_none", "visibility_clause": "ci_visibility_none"}
-- fixture_setup:
...
-- test_sql:
CREATE INDEX idx_ci_part_nulls_2f3d2c4c ON t_ci_partitioned (name DESC NULLS FIRST) GLOBAL;
-- fixture_teardown:
DROP TABLE IF EXISTS t_ci_partitioned CASCADE;
